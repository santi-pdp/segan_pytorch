...
class GSEGAN(SEGAN):
    ...
    def gradient_penalty(self, real_data, generated_data, cond):
        """ Modified from: 
        https://github.com/EmilienDupont/wgan-gp/blob/master/training.py
        """
        batch_size = real_data.size(0)
        device = 'cuda' if real_data.is_cuda else 'cpu'
        ...
        D_in = torch.cat((interpolated, cond), dim=1)
        # Calculate probability of interpolated examples
        prob_interpolated, hact = self.D(D_in)

        # Calculate gradients of probabilities with respect to examples
        gradients = torch_grad(outputs=prob_interpolated, inputs=D_in,
                               grad_outputs=torch.ones(prob_interpolated.size()).to(device),
                               create_graph=True, retain_graph=True)[0]

        # Gradients have shape (batch_size, num_channels, time),
    ...
    def train(self, opts, dloader, criterion, l1_init, l1_dec_step,
        ...
        self.G.train()
        self.D.train()
        # If no fine-tunned frontends, use eval mode for BN statistics
        # of pre-training of each frontend
        if not self.G.ft_fe:
            self.G.frontend.eval()
        if not self.D.ft_fe:
            self.D.frontend.eval()
        """ Train the SEGAN """
        # create writer
        self.writer = SummaryWriter(os.path.join(opts.save_path, 'train'))
        ...
        for iteration in range(1, opts.epoch * bpe + 1):
            ...
            bsz = clean.size(0)
            for citer in range(self.critic_iters):
                # grads
                Dopt.zero_grad()
                ...
                gpenalty = self.gradient_penalty(clean, fake, noisy)
                d_loss = d_real.mean() - d_fake.mean() + gpenalty
                d_loss.backward()
                Dopt.step()
